...
0b. Phase 8 (ADR-136): run `npm run e2e` (payloads, the fourth `how_many, swipe_sort, pairs` lineup) and `supabase test db` locally for migrations `20260925000700`/`000800`, then `supabase db push` + `supabase test db --linked` before the final deploy (never on an event day); playtest all three with ≥ 5 strong players (HM-T14, SS-T14, PR-T14) and retune only with evidence; real-device checks pending (HM-T13, SS-T13); Arabic review of the new strings (COPY §5.8–§5.10, OQ-04); confirm the host reveal (WP4, `src/host/HowManyReveal.tsx`) is merged and `Intermission.tsx`'s `REVEALS` map covers `how_many`.
