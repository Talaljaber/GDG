...
Related: ADR-021, ADR-022, ADR-105, ADR-113, ADR-134, ADR-136, ADR-138.
...
| How Many? | 3 flashes | 15 s per answer | 1.5 intro + 3 × (1 look + 2.5 flash + 15 + 0.5 locked) = **58.5 s** (`worstCaseMs` 60 000, ADR-138); a fixed schedule from `roundStartEpoch`, so a lock or hidden page never extends it (ADR-137 (2)) |
| Swipe Sort | items inside a 30 s game clock | item window `I(t)` 1100 → 600 ms (ADR-139) | 1.5 intro + 30 ≈ **32 s** (the 0.15 s gaps run inside the 30 s); the clock starts at `roundStartEpoch` + 1.5 s whatever the mount time (ADR-137 (3)) |
...
- Three flashes of 2.5 s, true counts `N_i` from the bands 4–7, 9–13, 14–18 (every integer; ADR-138), guesses `g_i` (null = no answer).
...
- The band widens with the flash (Weber's law), so a good player scores similarly on all three.
- Examples (`N = [6, 11, 16]`): guesses 6, 10, 14 → 905; 5, 9, 13 → 617; 4, 8, 11 → 260; 6, null, 15 → 657; 6, 11, 16 → 1000; 6, 11, 15 → 991; no answers → 0.
- Calibration assumptions (ADR-138, retuned after the 2026-09-26 playtest; formula and constants unchanged from ADR-136): with at most 18 chevrons shown for 2.5 s, **exact counts are common** (flash 1 is subitizing, flash 2 counting in groups), and ±1 is already outside the 5 % dead zone on every flash (1/18 ≈ 5.6 %), so a flash's full marks mean the exact count. A **strong** booth player is exact / ±1 / ±2 → **≈ 905**; exact / exact / ±1 → ≈ 990; typical ±1 / ±2 / ±3 → ≈ 617. **1000** (all three exact) is **reachable** and counts as normal play: the calibration no longer keeps 1000 out of reach (the old "out of reach without luck" assumption held only for 1 s flashes of 40–70 items), so ties at the top are settled by the earlier score (§5).
...
- Item window `I(t) = round(1100 − 500 × t / 30000)` ms for an item whose onset is at `t` on the 30 s game clock (1100 → 850 → 600 ms, ADR-139; was 900 → 450 ms); a 0.15 s gap follows every item.
...
- Examples: 43 / 1 / 5 at 450 ms → 864; 33 / 3 / 8 at 550 ms → 514; 22 / 5 / 12 at 640 ms → 113; a random spammer 30 / 33 / 0 → 0; idle (0 / 0 / 30: the epoch-exact timeline fits 30 missed windows in 30 s; 37 on the old window) → 0; 46 / 1 / 2 at 430 ms → 1000.
- Calibration assumptions (ADR-136, on the original 900 → 450 ms window): a **strong** player registers swipes at ≈ 450 ms (SD 80) with 2 % wrong: ≈ 49 items, misses concentrated in the last third where `I(t) < 550` → ≈ 43 / 1 / 5 → **≈ 864**. Typical (550 ms, 5 %) ≈ 33 / 3 / 8 → ≈ 514. **1000** needs `22 × net + B ≥ 999.5` (net 43 at ≤ 430 ms, net 44 at ≤ ≈ 540 ms, or net ≥ 46).
- Calibration with the ADR-139 window (1100 → 600 ms): the formula and the swipe times are unchanged (a swipe ends its item early, so cadence = swipe time + 150 ms); what changes is the misses. Slow players miss far fewer items, and so does a strong player, whose misses were the late items with `I(t) < 550`: with a 600 ms floor only a swipe slower than ≈ 600 ms misses (≈ 3 % at SD 80). Re-modelled with the same assumptions (Monte Carlo, swipe times normal and capped at the window): strong ≈ 49 / 1 / 0 → **1000 (capped)** in most runs; typical ≈ 39 / 2 / 2 → **≈ 815** (was ≈ 514); weak (640 ms, SD 120, 10 %) ≈ 30 / 3 / 5 → ≈ 490. So 1000 is now within a strong player's reach; the worked examples above are formula checks and keep their scores. Retune the 22 per net only with playtest evidence (SS-T14, ADR-139).
...
| How Many? | `{"rounds":[{"true_count":int,"guess":int\|null,"answer_ms":int\|null,"timed_out":bool}, …×3]}` | exactly 3 rounds with those types [`hm.shape`]; `true_count` in the flash's band 4–7, 9–13, 14–18 (ADR-138; during the rollout the server also accepts the old bands 8–15, 20–35, 40–70, so a phone still on the previous build is never rejected, migration `20260926000100`), `guess` 0–999, `answer_ms` 0–15000 [`hm.range`]; `answer_ms` null iff `timed_out`, and `guess` null ⇒ `timed_out` [`hm.timeout`]; not timed out ⇒ `answer_ms ≥ 300` [`hm.too_fast`]; no "too perfect" check (removed by ADR-138: three exact counts are normal play); `score ≤ round(1000 × Σ s_i / 3) + 1` with `s_i` as §3.8 (0 for a null guess; upper side only, under-reporting only hurts the sender) [`hm.formula_band`] |
| Swipe Sort | `{"correct":int,"wrong":int,"missed":int,"mean_swipe_ms":int\|null}` | malformed object/types [`ss.shape`]; `correct` 0–120, `wrong` 0–120, `missed` 0–80, `mean_swipe_ms` 0–1100 (the longest item window, ADR-139; was 0–900) [`ss.range`]; `mean_swipe_ms` null iff `correct = 0` [`ss.rt`]; `correct = 0` ⇒ `score = 0` [`ss.zero`]; `mean_swipe_ms ≥ 200` [`ss.too_fast`]; `correct × (mean_swipe_ms + 150) ≤ 30150` [`ss.too_many`]; `clamp(22 net, 0, 1000) ≤ score ≤ clamp(22 net + 60, 0, 1000)` with `net = correct − wrong − missed` [`ss.formula_band`] |
