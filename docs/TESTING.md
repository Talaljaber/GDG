...
- Calibration playtest (HM-T14, SS-T14, PR-T14): ≥ 5 strong players, 2 runs each after one practice run; record the scores in `PROGRESS.md`. Pass for Swipe Sort and Pairs: median 780–900, nobody at 1000; otherwise retune the constants (22/60/450, 6/12/80) and note it in ADR-136. How Many? (ADR-138): 1000 (three exact counts) is reachable by design; record the spread and retune `D`/`W` or the bands only with evidence (e.g. most strong players at 1000, or typical players below ≈ 500), with a note in ADR-138.
