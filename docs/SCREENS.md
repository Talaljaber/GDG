...
**P0 Shell (every player screen).** Top bar: GDG logo (small, static), language toggle (hidden during rounds, E17). System banners from §4 overlay the top.
...
- Host controls (inline-end bottom corner, every host screen): the screen action (Start / End round / New session), the language toggle and Sign out (the rest of H6 comes later).
