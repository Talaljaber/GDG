...
**P6 Game.** One of the ten games (the five originals, Close the Brackets and Color Clash (ADR-134), and How Many?, Swipe Sort and Pairs (ADR-136)); states and strings in `docs/games/<game>.md` §6 or §7 and `COPY.md` §5. Shared v2 chrome only: each game's intro card is eyebrow (`game.<id>.intro`) + title + pitch, progress labels ("Try n of 3", "Grid n of 3", "Length n", "n / 5", "Closed n", "Correct n", "Flash n of 3", "Sorted n", "Pairs n of 8") are small muted labels, titles use medium weight, chips are small-radius tags; every game spec (sizes, timings, colours, pad shapes, touch targets) is unchanged.

P6 for the ADR-136 games (full states in their docs §6): How Many? shows a 4 x 4 / 5 x 5 / 6 x 6 field of chevrons (4–7, 9–13, 14–18 of them) for a 2.5 s flash, then a digit pad with a 15 s countdown (ADR-138) (`game.how_many.look`/`.question`/`.ok`/`.backspace`, true counts never shown on the phone, only the big screen §5); Swipe Sort shows one chevron at a time in the middle of a full-bleed swipe surface with a blue-left/amber-right catch label in each bottom corner (`game.swipe_sort.zone_blue`/`.zone_amber`, the chevron follows the finger horizontally during the drag); Pairs shows a 4 x 4 face-down grid, tapping flips a card, a mismatch shows both for 0.7 s before flipping back (`game.pairs.found`, icon labels `game.pairs.icon.*`).
...
The breakdown block's rows are per game (`resultDetail.tsx`): the ADR-136 games add How Many? (one row per flash, `game.how_many.result_round` + `.result_guess`/`.result_missed`; still no true counts on the phone), Swipe Sort (`game.swipe_sort.result_correct`/`.result_wrong`/`.result_missed`/`.result_speed`) and Pairs (`game.pairs.result_pairs`/`.result_misses`/`.result_time`, or `.result_not_cleared` if the board wasn't cleared).
...
- **Bigger QR** (`host-qr-toggle`, on the rail beside Settings, `aria-pressed`): the stage shows only the QR, as large as the stage allows (`host-qr-full`), with the code and URL beside it; the players list is hidden until it's turned off. The same button (now `host.lobby.qr_smaller`), Escape or a click on the big QR brings the normal lobby back. Not remembered: every new lobby starts small.
...
Strings: `host.lobby.scan`, `host.lobby.or_visit`, `host.lobby.code_label`, `host.lobby.players`, `host.lobby.empty`, `host.lobby.empty_hint`, `host.lobby.join_title`, `host.lobby.step_scan`, `host.lobby.step_code`, `host.lobby.step_name`, `host.lobby.remove`, `host.lobby.remove_confirm`, `host.lobby.qr_bigger`, `host.lobby.qr_smaller`, `host.lineup.title`, `host.lineup.need`, `host.settings.title`, `host.settings.theme`, `host.settings.reduced_motion`, `common.lang_toggle`, `host.signout`, `game.trivia.unavailable`, `host.start`, `host.start_disabled_hint`, `game.*.name`, `common.cancel`, `common.confirm`.
...
Stop the Clock reveal: three strips (5 s, 10 s, 7 s targets, labelled with `game.stop_the_clock.target`), the time axis never mirrored in Arabic; one dot per player per measured guess at `50 % + 50 % × (guess − target) / 5 s`, dots beyond ±5 s pinned to the edge (outlined); the top 5 of the round board are labelled with their display names, each label placed in the first free lane (above, below, then a second row above/below) so neighbouring names don't collide. `game.stop_the_clock.reveal_title` and a single `game.stop_the_clock.reveal_axis` label over the centre line head the strips. Dots burst in strip by strip within 5 s of `rounds.ended_at` (`DESIGN_SYSTEM.md` §6.2; 200 ms fade-ins with reduced motion) — this data reveal is the one place rows still stream in deliberately. Every delay counts from `ended_at` (host offset), not from the mount: a host that mounts or reloads mid-step shows every dot already due at once (no burst), and late rows (E22) appear at once (ADR-137 (5)).
How Many? reveal (games-v3 §5, ADR-136; `src/host/HowManyReveal.tsx`, layout in `howManyStrips.ts`): the same step after a How Many? round, on the same flat tracks. One track per flash, labelled `game.how_many.reveal_round` ("Flash n · 27", the true count as the label's hero) with `game.how_many.reveal_mean` ("Crowd average 25") under it; the true count is the most common `true_count` across the rows (identical on every row since ADR-137 (1); the mode guards against a tampered row only with ≥ 3 rows, with 1–2 rows the higher-ranked row's value wins) and sits on the centre line (`game.how_many.reveal_axis`, under `game.how_many.reveal_title`); one dot per guess at `50 % + 100 % × clamp((guess − count) / count, ±50 %)`, so the ends are ±50 % off and the ticks every 10 %; guesses beyond are pinned (hollow); no dot for a missed flash; the top 4 of the round board labelled in lanes as above (`HM_REVEAL_LABELLED`, ADR-138: with counts of 4–18 many top players type the exact count and their dots coincide on the centre line, and 4 labels always fit the 4 lanes). Dots burst in track by track within 3.5 s of `rounds.ended_at` (`HM_REVEAL_DOTS_MS`); at 4.0 s after `ended_at` (`HM_REVEAL_MEAN_MS`) the crowd average (the rounded mean of the guesses) fades in as an amber rule across each track, with its legend line (no shards; fades only with reduced motion), leaving ≥ 3 s of the 7 s step for the whole picture. A host that mounts or reloads mid-step shows everything already due at once (no burst); late rows (E22) appear at once (ADR-137 (5)). The axis never mirrors in Arabic. Phones never show the true counts. Test ids `hm-reveal`, `hm-strip` (`data-count`), `hm-dot`, `hm-dot-label`, `hm-mean`.
