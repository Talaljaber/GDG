...
-- + 'how_many', 'swipe_sort', 'pairs' (migration 20260925000700, ADR-136; see the notes below)
...
- `public.game_id` again (migrations `20260925000700_game_ids_v3.sql` + `20260925000800_score_bounds_v3.sql`, ADR-136): three labels appended, `how_many`, `swipe_sort` and `pairs`, in that order (additive; no row changes), ten games in all. Same two-step pattern: between the two migrations a score for any of the three fails with `GD008 game.unknown`.
...
- Swipe Sort window (migration `20260926000200`, ADR-139): same `create or replace` and revoke, body copied from `20260926000100`; only the `swipe_sort` branch changes: `mean_swipe_ms` 0–1100 (was 0–900), the longest item window now that it ramps 1100 → 600 ms. `ss.too_many` (`correct × (mean_swipe_ms + 150) ≤ 30150`), `ss.formula_band` and `missed ≤ 80` don't depend on the window and are unchanged (an idle player now misses 30). The bound only widens, so phones on the previous bundle keep saving. pgTAP `11_score_bounds_swipe_sort.sql`.
...
| `keepalive() returns void` | anon (and signed-in) | `update keepalive set pinged_at = now() where id = 1`. Called by an external scheduler every 6 h (ADR-031; no GitHub Actions). | — |
