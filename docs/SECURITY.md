...
| T13 | **Supabase project pause** mid-event | Low once the keepalive is set up | Keepalive every 6 h via an external scheduler POSTing to `rpc/keepalive` with the publishable key (ADR-031, changed 2026-09-25: no GitHub Actions, so the old public-repo 60-day schedule rule no longer applies); check `keepalive.pinged_at` on dry-run day | Not yet set up (`PROGRESS.md`); until it is, the Free project can pause after ~7 days idle | Availability, not confidentiality |
...
| T18 | **Join-code enumeration** by a script (T6's "1 in 9,000" is per guess) | Low–medium (was medium; mitigated 2026-09-24) | **Wrong-code throttle** (ADR-130, migration `20260925000400_join_throttle.sql`, pgTAP `08_join_throttle.sql`): `join_session` returns a wrong code as data and logs it per `auth.uid()` in `private.join_attempts` (closed to anon and guests); 5 wrong codes in a rolling 60 s lock that identity for 30 s, even for the right code. Keyed on `auth.uid()`, never the IP (shared carrier NAT). Also: codes change every session; joins lock at Start (T2); names still pass the filter; the host removes strays | Each fresh anonymous sign-in gets 5 more guesses, so one IP gets about **5,000 guesses/h** (1,000 sign-ins/h, ADR-102) instead of all 9,000 codes in ~25 s: a script can still find a live code in roughly 1–2 hours from one IP, faster from several, and then join bots or submit plausible scores (T1). Each attempt also leaves an anonymous user behind (§6 cleanup) | The team kept 4-digit codes (chat 2026-09-24). The throttle turns seconds into hours for one IP; the upgrades are Turnstile on anonymous sign-in (ADR-124) or longer codes (ADR-003, ADR-111) |
...
- [ ] Migration `20260925000400_join_throttle.sql` (T18, ADR-130) applied the same way, **after** the site that reads `join_session`'s data results is live (that client handles both the old raised `GD001` and the new data results; an older build would treat `{"error": …}` as a successful join).
...
**Open, ranked:** T18 code enumeration (mitigated by the wrong-code throttle, ADR-130; residual low–medium: ~5,000 guesses/h per IP through fresh anonymous sign-ins, accepted); T20 Realtime quota abuse (low–medium, accepted); T19 public-channel presence spoofing (low, accepted); the informational items below.
