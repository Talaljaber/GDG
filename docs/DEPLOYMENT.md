...
- Later pushes: `20260925000500`/`000600` (ADR-134) and `20260925000700`/`000800` (ADR-136) are on the cloud (2026-09-25). **Not yet on the cloud:** `20260926000100_how_many_retune` (ADR-138; widens the How Many? bounds and keeps the old bands, so it must be pushed **before** the Netlify build that uses the new bands: the cloud still rejects their counts and 10–15 s answers) and `20260926000200_swipe_sort_window` (ADR-139; widens Swipe Sort's `mean_swipe_ms` bound to 1100, so it too must be pushed **before** the Netlify build with the 1100 → 600 ms window: the cloud still rejects a mean above 900 ms with `GD008 ss.range`). Push both, then build `main`.
