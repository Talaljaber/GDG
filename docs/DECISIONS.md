...
Context: five games make repeat visitors see the same lineups; the brief adds games under the same contract. Decision: (1) Two new games, `close_brackets` (`games/close-brackets.md`) and `color_clash` (`games/color-clash.md`): touch only, 1.5 s intro + a 30 s game clock (worst case ≈ 32 s), per-attempt timeouts (10 s per bracket sequence, 3 s per Stroop trial), a 0–1000 integer computed on the phone. (2) **Seeding already exists per round**: every game gets the round's seed through `GameProps.seed` (`src/lib/rng.ts`), so no seeding ADR is needed; the new games key their draws by content position (bracket sequences by length + index at that length, Stroop trials by index) so everyone in a round sees the same sequence and a reload shows the same one. (That claim was wrong: the shell seeded per player until ADR-137 (1), which implements it.) (3) Formulas and calibration in `SCORING.md` §3.6–§3.7 (strong ≈ 840 / ≈ 820, 1000 out of human reach); server bounds in §4 with band checks like Simon's, added by migrations `20260925000500` (enum labels) and `20260925000600` (bounds; separate because a new enum label can't be used in its own transaction). (4) Brief deviations: no red (a wrong closer is an ink shake, as Trivia uses ink for wrong); Color Clash's inks are the brand blue, amber and ink (not the brief's hex values; `DESIGN_SYSTEM.md` §0 and §2.5 win), checked under deuteranopia and protanopia by `npm run contrast`; the Arabic colour words need the Arabic review (OQ-04). (5) The picker, day boards, dashboard and registry are data-driven: the new games appear with no special-casing; the dashboard's game filter now reads the shared `GAME_IDS` list (`src/games/types.ts`). The picker and leaderboard tabs are **not** redesigned (the brief's "Changes to existing parts" stay out of scope). Consequences: the pool is 7 games; the host picker shows 4 "+ add" buttons beside 3 picks (it scrolls sideways if needed); the constants (15 / 100 and 25 / 75) are retuned only with playtest evidence (`TESTING.md` §4) and a note here; no existing game changes behaviour.
...
Context: with seven games repeat visitors still meet the same lineups; the brief's remaining touch games fit the contract, and Steady Hand does not (motion permission). Decision: (1) Three new games, `how_many` (`games/how-many.md`), `swipe_sort` (`games/swipe-sort.md`) and `pairs` (`games/pairs.md`): touch only, 1.5 s intro, worst cases 39 / 32 / 62 s, per-attempt timeouts (10 s per answer; a 900 → 450 ms item window, 1100 → 600 ms since ADR-139; the 60 s board), a 0–1000 integer computed on the phone; seeded per round as ADR-134 (2) (`hm:round<i>`, `ss:item<k>`, `pr:layout`). Implemented from ADR-137 (1); before that the shell seeded per player. (2) Formulas and calibration in `SCORING.md` §3.8–§3.10 (strong ≈ 815 / 864 / 825; 1000 needs ≤ 5 % on three flashes, net ≥ 43 at ≤ 430 ms, or a clean clear in ≤ 15 s); bounds in §4 (`hm.*`, `ss.*`, `pr.*`) by migrations `20260925000700` (labels) and `20260925000800` (bounds). How Many?'s numbers here (39 s, 10 s, ≈ 815, 1000 out of reach, `hm.too_perfect`) are retuned by ADR-138. (3) **How Many? reveal** on the big screen: the H3 round-board step shows three count strips (true count centred, ±50 % relative window, top 5 labelled, dots in shatter bursts within 5 s, 3.5 s from `ended_at` since ADR-137 (5)) plus an amber crowd-average marker; phones never show the true counts (they are shared by the whole room), only the player's own guesses; ADR-025 stays Stop the Clock's. (4) Brief deviations: Swipe Sort's window floors at 450 ms, not ≈ 350 ms (choice reaction time plus swipe travel; superseded by ADR-139: the window is now 1100 → 600 ms), and its chevrons point in the swipe direction as a shape cue beside colour; How Many? counts are never multiples of 10 (dropped, and the top-5 reveal labels become top 4, by ADR-138); Pairs scores partial boards (`730 − 80 × missing pairs`) so a typical guest isn't at 0. (5) Swipe Sort defends against browser gestures (`touch-action: none`, `touchmove.preventDefault()`, `overscroll-behavior: none` while mounted, a 24 px edge guard); iOS edge-swipe-back cannot be blocked and is tested on real devices (SS-T13). (6) Pairs icons are hand-drawn SVG in the brand line style; no product logos. Consequences: the pool is 10 games; the picker, boards and dashboard stay data-driven; constants (`D`/`W`, 22/60/450 (the window is ADR-139's now), 6/12/80) are retuned only with playtest evidence (`TESTING.md` §4) and a note here; no existing game changes behaviour.

### ADR-137 Phase 8 timing and seeding fixes
**Proposed**, except (2): **Accepted** (confirmed by the team in chat 2026-09-26) · investigation `docs/investigations/2026-09-25-phase8-games/` (2026-09-25)
Context: the shell seeded every game per player (`<round id>:<player row id>`, `src/player/PlayerApp.tsx`), against ADR-134 (2) and ADR-136 (1), so each phone drew its own How Many? fields, Pairs board and sequences; How Many?, Pairs and Swipe Sort chained steps on the time a late timer fired; Pairs measured `clear_ms` on `performance.now()`, which stops during a device sleep; the H3 reveal counted from its mount, not `ended_at`. Decision: (1) **Seed scope**: `GameModule.seedScope` is `'round'` (default: seed = the round id, every phone in a round sees the same fields, boards and sequences) or `'player'` (`<round id>:<player row id>`; only Trivia, ADR-027). This corrects ADR-134 (2)'s "seeding already exists per round". (2) **How Many? fixed schedule**: look 1 starts at `roundStartEpoch` + 1.5 s; flash window look + 1 … + 2 s; the answer ends at the OK tap or + 10 s; locked = answer end + 0.5 s; the next look starts at the lock's end. Timers never move an anchor: after a reload, screen lock, freeze or hidden tab, every step whose epoch passed is closed with its rule on return (a passed look skips its flash as before, ADR-136; a passed answer is a timeout). A hidden page never pauses the schedule: flashes shown while the page is hidden are lost, and the round still ends inside its 39 s worst case (E15, ADR-018). The schedule stands; its durations (the + 1 … + 2 s flash window, + 10 s answer, 39 s) are superseded by ADR-138 (+ 1 … + 3.5 s, + 15 s, 58.5 s). (3) **Pairs and Swipe Sort board clocks** start at `roundStartEpoch` + 1.5 s whatever the mount time; Pairs' `clear_ms` = `Date.now()` at the eighth match minus `gameStartEpoch` (the clock of the 60 s end, so a sleep never shortens it); Pairs closes a passed lock or end on `visibilitychange`/`pageshow` like the other games; a timed-out board turns unfound cards face down. (4) **Swipe Sort drag**: a drag moves only the chevron of the item it started on; when that item ends, the drag offset resets and the rest of the touch counts nothing. (5) **H3 reveal anchoring**: dot and crowd-average delays count from `rounds.ended_at` (host offset); late rows (E22) and a reloaded host show whatever is already due at once; How Many? dots within 3.5 s and the average at 4.0 s (`HM_REVEAL_DOTS_MS`, `HM_REVEAL_MEAN_MS`), Stop the Clock dots within 5 s; the 7 s step is unchanged (ADR-117/129). (6) The "one tampered row can't move the axis" rule needs ≥ 3 rows; with 1–2 rows the higher-ranked row's `true_count` is shown (accepted, ADR-021). (7) **Shell**: a round that ends (server `done`) while the phone is still in its 3-2-1 counts as not played: no game mounts, nothing is submitted (ADR-014), and the phone clears that round's local state (`roundId`, `roundStartEpoch`, `seed`, snapshot) so its round clock stops; while a round is on the phone and not yet submitted (the 3-2-1 included) the phone refetches its round rows every 5 s (one small request, `ROUND_SAFETY_REFETCH_MS`; the 15 s full refetch stays), so a missed round-end event is caught well inside the 15 s late window. Consequences: worst cases unchanged (`SCORING.md` §2); no migration (bounds unchanged; the Swipe Sort idle example is 37 misses, 30 since ADR-139); the 128 s hold on short rounds is OQ-23 (ADR-017 is Accepted); docs `games/how-many.md` §3/§9, `games/pairs.md` §3/§4/§9, `games/swipe-sort.md` §3/§9, `games/trivia.md` §2, `SCREENS.md` H3, `SESSION_LIFECYCLE.md` §4.1/§5.

### ADR-138 How Many? retune after playtest
**Accepted** · requested by the team in chat 2026-09-26 (Talal, after playing it: "needs more time for each question; too many particles; keep the first 4–7, the second 9–13, the third 14–18; increase the time, I couldn't even see them")
Context: with 40–70 chevrons shown for 1 s the fields were too dense and too short to see, let alone estimate; the game felt unfair rather than hard. Decision: (1) Count bands per flash **4–7, 9–13, 14–18**, every integer (the "never a multiple of 10" rule of ADR-136 (4) is dropped). (2) Grids **4 × 4, 5 × 5, 6 × 6** (`HM_GRID`, was 5 / 7 / 10), so the chevrons are drawn much bigger (≈ 57 / 46 / 38 px on a 360 px phone); the glyph/jitter rule is unchanged (a glyph never leaves its cell). (3) Flash **2.5 s** (`HM_FLASH_MS`, was 1 s); answer timeout **15 s** (`HM_ANSWER_MS`, was 10 s); look 1 s, locked 0.5 s, intro 1.5 s and the fixed schedule of ADR-137 (2) unchanged (flash window look + 1 … + 3.5 s). (4) Scoring formula unchanged (`D = 0.05`, `W = 0.30 / 0.40 / 0.50`). (5) The `hm.too_perfect` rejection is removed: three exact counts are normal play now. (6) The H3 reveal labels the top **4** (`HM_REVEAL_LABELLED`, was 5): exact guesses coincide on the centre line, and 4 labels always fit the 4 label lanes. (7) The intro string reads "3 flashes · 2.5 seconds each" (`COPY.md` §5.8; Arabic in the OQ-04 review). Consequences: worst case 1.5 + 3 × (1 + 2.5 + 15 + 0.5) = **58.5 s** (`worstCaseMs` 60 000), still far inside the 120 s cap; the calibration no longer keeps 1000 out of reach (strong ≈ 905, typical ≈ 617, all three exact = 1000; ties at the top go to the earlier score), so HM-T14 records the spread instead of "nobody at 1000"; server bounds by migration `20260926000100` (new bands, `answer_ms` ≤ 15 000, no `too_perfect`), which also accepts the old bands during the rollout so a phone still on the previous build is never rejected; `SCORING.md` §2/§3.8/§4, `games/how-many.md`, `SCREENS.md` P6/H3, `TESTING.md`; ADR-136 (2)/(4) and ADR-137 (2) keep their text with pointers here.

### ADR-139 Swipe Sort longer item window
**Accepted** · requested by the team in chat 2026-09-26 (Talal: "the swipe game left and right, increase the time for each one a bit")
Context: after playing it, the team found the 900 → 450 ms window (ADR-136 (4)) too short per item. Decision: (1) The item window ramps linearly from **1100 ms to 600 ms** over the 30 s game clock, `I(t) = round(1100 − 500 × t / 30000)` (`SS_WINDOW_START_MS` / `SS_WINDOW_END_MS`, `src/games/swipe-sort/scoring.ts`; was 900 → 450): 1100 / 850 / 600 ms at 0 / 15 / 30 s. (2) Unchanged: the 150 ms gap, the 30 s clock and 1.5 s intro (worst case ≈ 32 s), 22 points per net, the speed bonus (60 max, full at ≤ 400 ms, 0 at ≥ 700 ms) and the formula; the timeline stays epoch-exact (ADR-137). (3) Server bound `mean_swipe_ms` 0–**1100** (the longest window; was 0–900) by migration `20260926000200_swipe_sort_window.sql`; `ss.too_many` (`correct × (mean + 150) ≤ 30150`), the formula band and `missed ≤ 80` don't depend on the window and are unchanged. The bound only widens, so phones on the previous bundle keep saving. Consequences: an idle player misses **30** items (was 37; worked example E = 0 / 0 / 30, pgTAP 11); a 10 s screen lock ≈ 8–12 misses; the other worked examples keep their scores. A swipe still ends its item at once, so fast swipes are timed as before, but misses drop for everyone: re-modelled with the ADR-136 assumptions, a strong player (450 ms, SD 80) goes from ≈ 43 / 1 / 5 (≈ 864) to ≈ 49 / 1 / 0 → **1000 (capped)**, typical ≈ 514 → ≈ 815, weak ≈ 490. So "nobody at 1000" (SS-T14, AC8.10) is expected to fail; the playtest decides whether to lower the 22 per net (which needs a new bounds migration for `ss.formula_band`), OQ-24. Deploy: `20260926000200` (and ADR-138's `20260926000100`) must be on the cloud before `main` builds with the new window, or a mean above 900 ms is rejected (`GD008 ss.range`). `SCORING.md` §2/§3.9/§4, `games/swipe-sort.md`, `TESTING.md`, `DATA_MODEL.md` §5, `DEPLOYMENT.md` §2.7; ADR-136 (1)/(4) keep their text with pointers here.
