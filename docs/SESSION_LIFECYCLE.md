...
| E6 | **Late join during play** | Guest types the corner code → joins the pending session → "You're in the next round". Typing the *running* session's old code → `GD001` "That code isn't active — use the code on the screen now." Every wrong code counts toward the throttle (ADR-130): after 5 within 60 s, `join_session` answers `GD013` for 30 s, even for the right code; P2 counts down with Join disabled, keeping name and code. |
