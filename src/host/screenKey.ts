 ...
 * host-v3 §4.4): H1 → H2 (round start), H2 → H3, H3 → H2 (next round start),
 * H3 → H4. An intermission is **one** key per round: its steps crossfade
 * inside the stage (Intermission.tsx), so the board stays mounted and FLIPs
 * from the round order to the total order; the step is `data-step` on
 * `host-intermission`. H4 and H5 share one key: the day-board merge plays
 * between them.
 */
export function hostScreenKey(host: Pick<HostController, 'data' | 'screen' | 'intermission'>): string {
  ...
  switch (screen) {
    ...
    case 'intermission':
      // spinner until the server offset is known
      return intermission ? `intermission:${intermission.round.id}` : LOADING_KEY;
    case 'results':
    case 'dayboard':
