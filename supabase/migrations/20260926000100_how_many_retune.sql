-- 20260926000100 How Many? retune (ADR-138, team decision 2026-09-26).
-- Spec: docs/SCORING.md section 4 (bounds and reason codes), docs/games/how-many.md,
...
-- calling it unchanged. The body is copied from 20260925000800_score_bounds_v3.sql (the last
-- definition); only the How Many? branch changes:
--   * true_count per flash: [4,7], [9,13], [14,18] (every integer, 10 allowed). The old bands
--     (8-15, 20-35, 40-70) stay accepted as well, so phones on the currently deployed bundle keep
--     saving during the rollout; a later migration may drop them once no old bundle is in use.
--   * answer_ms 0-15000 (the answer timeout is now 15 s; was 0-10000).
--   * the 'hm.too_perfect' rejection (three exact guesses) is removed: three exact guesses are now
--     normal play.
-- The formula band (D 0.05, W 0.30/0.40/0.50, 'hm.formula_band') and every other game's branch are
-- unchanged. The server still never recomputes a score (ADR-113).
--
...
comment on function private.score_bounds_violation(public.game_id, integer, integer, jsonb) is
  'SCORING.md section 4: null when a submission is plausible, else a reason code (GD008 detail). Ten games (ADR-136; How Many? retuned by ADR-138).';
